...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")
        await self.highrise.walk_to(Position(17.5 , 0.35 , 11.5 , "FrontRight"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala")   
        await self.highrise.chat(f"Bienvenue {user.username} dans 🏖️PLAGE🏖️ de ❤️RENCONTRE❤️.Pour danser utilise de 1 à 95.Ecris help pour voir la liste des commandes")
